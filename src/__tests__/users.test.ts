...
describe('User Integration Tests', () => {
  ...
  it('should list all users for authenticated users', async () => {
    const response = await axiosInstance.get(`${process.env.BASE_URL_LOCAL}/users`);
  ...
  it('should not list users for unauthenticated requests', async () => {
    ...
    await expect(axiosInstance.get(`${process.env.BASE_URL_LOCAL}/users`)).rejects.toThrow();
  ...
  it('should allow a user to delete their own account', async () => {
    const deleteResponse = await axiosInstance.delete(`${process.env.BASE_URL_LOCAL}/users/${setupInfo.userId}`);
  ...
  it('should not allow a user to delete another user\'s account', async () => {
    ...
    await expect(axiosInstance.delete(`${process.env.BASE_URL_LOCAL}/users/${anotherUserId}`)).rejects.toThrow();
  });
});
